...
import rc.so.entity.Check2;
import rc.so.entity.Check2.Fascicolo;
import rc.so.entity.Check2.Gestione;
import rc.so.entity.Check2.VerificheAllievo;
import rc.so.entity.FadCalendar;
import rc.so.entity.Presenti;
import rc.so.util.CompilePdf;
import rc.so.util.ExportExcel;
import static rc.so.util.ExportExcel.lezioniDocente;
import static rc.so.util.ExportExcel.oreFa;
import static rc.so.util.ExportExcel.oreFb;
import rc.so.util.ImportExcel;
import static rc.so.util.MakeTarGz.createTarArchive;
import rc.so.util.SendMailJet;
import static rc.so.util.Utility.ctrlCheckbox;
...
//            }
    ...
    protected void cambiaDocReportFad(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            ex.printStackTrace();
            e.insertTracking(String.valueOf(((User) request.getSession().getAttribute("user")).getId()), "OperazioniMicro caricaregistroFAD: " + estraiEccezione(ex));
            resp.addProperty("result", false);
